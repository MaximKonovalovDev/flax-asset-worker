...
def all_no_key_cmd(
    ...
    compact: Annotated[
        ...
        typer.Option(
            "--compact",
            help=(
                "v1.35.s203: single-line JSON output. No effect without --json."
            ),
        ),
    ] = False,
    json_out: Annotated[bool, typer.Option("--json")] = False,
...
def all_key_cmd(
    ...
    html_out: Annotated[
        ...
        typer.Option(
            "--html",
            help=(
                "v1.49.s256: write standalone HTML keyed-fan-out report"
                " (per-provider status, including skipped/no-key). Preempts JSON/text."
            ),
        ),
    ] = Path(""),
    csv_out: Annotated[
    ...
    open_html: Annotated[
        ...
        typer.Option(
            "--open",
            help=(
                "v1.49.s256: with --html, auto-open in system browser."
            ),
        ),
    ] = False,
    compact: Annotated[
        ...
        try:
            ...
            for p in providers_run:
                if p.get("ok"):
                    ...
                    status_class = "b-grey"; status_text = "skipped (no key)"
                else:
                    status_class = "b-red"; status_text = "failed"
                rows_html.append(
                    "<tr>"
            ...
            html = (
                ...
                "</style></head><body>"
                "<h1>FAW Fan-out (keyed R1A providers)</h1>"
                "<p class='summary'>"
                f"Query: <code>{query}</code> &middot; "
                f"dry-run: {dry_run} &middot; parallel: {parallel} &middot; "
                f"include-video: {include_video} &middot; "
                f"providers run: {summary['providers_run']} &middot; "
                f"ok: {summary['providers_ok']} &middot; "
                f"skipped: {summary['providers_skipped']} &middot; "
                f"total matched: {summary['total_matched']} &middot; "
                f"total downloaded: {summary['total_downloaded']}"
                "</p>"
                "<table>"
            ...
            print(f"gen_all_key_error=html_write_failed: {exc}")
            raise typer.Exit(code=1)
        print(f"gen_all_key_html_path={html_path}")
        if open_html:
            import platform
            import subprocess
            sys_name = platform.system().lower()
            try:
                if sys_name == "windows":
                    ...
                    subprocess.run(["xdg-open", str(html_path.resolve())], check=False)
                print("gen_all_key_html_opened=true")
            except Exception as exc:
                print(f"gen_all_key_html_open_failed={exc}")
        return
